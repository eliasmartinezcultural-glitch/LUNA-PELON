...
export function createGameEngine({ world, npcs, discovery, mission, storage = localStorage, onMessage }) {
  ...
  const navigation = createNavigationSystem({
    worldModel,
    isBlocked: (x, y, radius) => !canOccupy(worldModel, x, y, radius, registry.all().filter((entity) => entity.type === 'npc')),
  });
  ...
  function update(dt) {
    ...
    if (moved) {
      ...
      for (const npc of registry.all().filter((entity) => entity.type === 'npc')) {
        if (near(state, npc, 52)) show(`Presioná ESPACIO para hablar con ${npc.name}.`);
      }
