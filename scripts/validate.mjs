...
const required = ['index.html','styles.css','src/main.js','src/data.js','src/content/memories.js','src/presentation/world-renderer.js','src/core/runtime-contract.js','src/core/state.js','src/core/event-bus.js','src/core/engine.js','src/core/world.js','src/core/entity.js','src/systems/input.js','src/systems/collision.js','src/systems/transitability.js','src/systems/spatial.js','src/systems/interaction.js','src/systems/persistence.js','src/systems/navigation.js','src/systems/npc.js','src/systems/time.js','src/systems/dialogue.js','src/systems/mission.js','tests/core-contract.test.mjs','README.md','ARCHITECTURE.md','CORE-CONTRACT.md','CHANGE-PROTOCOL.md','package.json'];
for (const file of required) if (!fs.existsSync(file)) throw new Error(`Missing required file: ${file}`);
const read = (file) => fs.readFileSync(file, 'utf8');
const html=read('index.html'), data=read('src/data.js'), content=read('src/content/memories.js'), main=read('src/main.js'), renderer=read('src/presentation/world-renderer.js'), engine=read('src/core/engine.js'), state=read('src/core/state.js'), world=read('src/core/world.js'), entity=read('src/core/entity.js'), collision=read('src/systems/collision.js'), transitability=read('src/systems/transitability.js'), spatial=read('src/systems/spatial.js'), interaction=read('src/systems/interaction.js'), persistence=read('src/systems/persistence.js'), navigation=read('src/systems/navigation.js'), npc=read('src/systems/npc.js'), time=read('src/systems/time.js'), dialogue=read('src/systems/dialogue.js'), mission=read('src/systems/mission.js'), pkg=JSON.parse(read('package.json'));
...
if (!data.includes("VERSION='v0.9.0'") || !data.includes("./content/memories.js")) throw new Error('v0.9 content boundary contract failed');
if (!content.includes('MEMORIES') || !content.includes('verification')) throw new Error('Narrative content registry contract failed');
...
if (!data.includes('steps:') || !data.includes('meet-marta') || !data.includes('discover-memory')) throw new Error('Mission content contract is incomplete');
...
for(const [file,contentText] of [['index.html',html],['CORE-CONTRACT.md',read('CORE-CONTRACT.md')],['ARCHITECTURE.md',read('ARCHITECTURE.md')],['CHANGE-PROTOCOL.md',read('CHANGE-PROTOCOL.md')]]) if(!contentText.includes(version)) throw new Error(`${file} version mismatch: expected ${version}`);
